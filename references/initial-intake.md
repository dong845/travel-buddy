...
The scope question asks how far the traveller will go in terms of visa effort (`home_country_only`, `visa_free_only`, `any_including_visa`), not whether the trip is domestic or cross-border. Intakes saved with the older `domestic` / `cross_border` / `domestic_or_cross_border` values are still accepted and mapped forward. Acceptable visa effort is derived from that same answer instead of being asked separately, and per-traveller identity is prefilled from the profile's nationality, residence country, and residence-status category.
