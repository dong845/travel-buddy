...
- For cross-border candidates only: passport nationality, country of legal residence, and whether every traveller’s passport stays valid at least six months past the trip (`valid_through_trip`, `not_sure`, or `needs_renewal`); visa tolerance (visa-free only, e-visa acceptable, or visa process acceptable). Ask for the status, never a document number, image, or expiry date. Do not request or treat this as a blocker for domestic-only travel.
