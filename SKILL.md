...
**Every plan answers the first hour on the ground, through the required `arrival_essentials` block** — `payment`, `connectivity`, `emergency`, `health_and_insurance`. Every other block in the contract is about the days; none of them answered *can I pay, can I get online, who do I call, am I insured*, and a page that schedules six days of meals and cannot say whether the card in the traveller's pocket will work at the first ticket machine has answered the easy half. Each entry is `researched` with a summary, an HTTPS `source_url` and a `checked_at`, or `unverified` with a summary saying so, or `not_applicable` **with a reason** — a domestic trip needs no consulate, and saying so is a different act from nobody having looked. A `researched` emergency entry that names no `local_emergency_number` is refused outright: it is the one field in this block somebody dials while something is going wrong. These are the facts recalled instead of checked, and recalled wrongly — the case that forced the block was a Chinese passport holder resident in the Netherlands flying to Switzerland, whose Dutch EHIC works across the EU and is worthless to *them* in Switzerland, because the EU regulation extending social-security coordination to third-country nationals does not apply there; nationality is the only thing separating those two answers and nothing in the plan would have asked. `new_plan_skeleton.py` emits the block with `TODO:` markers so it arrives as a form to fill rather than as a gate failure, the renderer prints it as its own panel because collected-and-not-shown is the same defect as never collected, and `validate_trip_html.py` requires the `#arrival-essentials` region on the page. The field-by-field rules are in the first-hour section of [references/booking-html-output.md](references/booking-html-output.md). **Cost: four facts, not a phase.** Give them to the agent already doing the entry domain in feasibility, inside its existing cap — they sit on that agent's own nationality × destination axis — and budget 4–6 lookups for the block per *country*, not per stop. Never commission a separate agent or a separate round for them; [references/research-budget.md](references/research-budget.md) sets out why, and why `unverified` with an honest summary is the correct way to run out of budget.
...
An edit to a plan that was already delivered verified — the traveller swaps a dinner, moves a morning — keeps its report: re-save it, and the gate names exactly the sections that moved since the verified save. Recheck those alone and record them in the report's `rechecks` (the scaffold's `--recheck` writes the entries); do not re-run the whole pass for one changed card, and do not save it as verified without the recheck.
...
- the returned intake was read and recorded in the shapes [references/initial-intake.md](references/initial-intake.md) defines — the scenery/activity taxonomy, the per-person budget model, the destination-scope rules, and the two feasibility answers that must come out as machine-readable values — rather than carried forward as prose the later gates cannot filter on;
...
- the booking-link, map-endpoint, closed-enum and HTML-contract rules in [references/booking-html-output.md](references/booking-html-output.md) were read *before* the page was built rather than rediscovered one gate rejection at a time — it is the largest file in the reference layer, about a third of it and the one the page gates name by anchor when they refuse a plan, so a rejection quoting `booking-html-output.md#…` means the rule was written down and not read;
- map, transport and booking providers were routed through [references/regional-service-routing.md](references/regional-service-routing.md) for the destination's own service market — including the mainland-China rules — rather than chosen for brand familiarity;
...
- every profile read, write, merge or deletion followed [references/profile-and-storage.md](references/profile-and-storage.md) — consent before a profile exists, its decision-precedence order, its place-history semantics, and `digital_travel_access` kept to convenience preferences with no account, document or payment context stored anywhere;
- the answer preserves choice and makes trade-offs legible;
- for a Discovery or Constrained-discovery task, the shortlist went through `python scripts/save_discovery_deliverables.py <shortlist.json> --workspace "<workspace>" --intake <intake.json>` and its `Shortlist JSON:` path was reported — with `--no-intake` only when no saved intake exists, and then said aloud, because that run never tested the winner against the traveller's own stated constraints and the saved file records the gap;
- `python scripts/check_plan_consistency.py <plan.json> --verification <report.json>` exits clean, so no route total, walking figure, meal placement, calendar date, or budget line contradicts the data it is derived from — and the report is checked against *this* plan while doing it. The bare form is refused, and `--no-verification-yet` is an iteration waiver, not a delivery state: a page delivered on it has had no report read at all;
