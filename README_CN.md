...
**它假定自己会出错，并让错误发出声音。** 结构校验只能证明页面「格式规整」，永远证明不了它「是真的」—— 一次真实运行通过了全部结构校验，同时交付了：只写到签证、漏掉 EVUS 登记的入境结论（中国护照持有人会因此在值机柜台被拒登机）、把同一架飞机当成两个航空公司比价、把免费导览排在它根本不开的那一天、把晚餐排进 17:00 就打烊的店、以及在旅客明确要求「避免长距离步行」的前提下，把实际最重的一天标成「最轻」。所以现在 `check_plan_consistency.py` 把「散文靠不住」的部分交给代码判定，五域核验负责只有真实世界能回答的部分。没跑核验就保存的计划，会在自己的首页上说明这一点。
